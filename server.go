...
func (s *Server) Open() error {
	var err error
	s.Db, err = sql.Open("mysql", "root:12345@tcp(127.0.0.1:3306)/portaldb?parseTime=true")
	if err != nil {
		log.Println(err)
		return err
	}
	...
	go func() {
		...
		/*
			...
			router.POST("/app-api/v1/user_login", func(c *gin.Context) {
				API_PostUserLogin(c)
			})
			router.POST("/app-api/v1/user_logout", func(c *gin.Context) {
				API_PostUserLogout(c)
			})
			router.POST("/app-api/v1/user_info", func(c *gin.Context) {
				API_GetUserInfo(c)
			})
			router.POST("/app-api/v1/user_addrs", func(c *gin.Context) {
				API_GetUserAddrs(c)
			})
			router.POST("/app-api/v1/user_addr", func(c *gin.Context) {
				API_PostUserAddr(c)
			})

			router.POST("/app-api/v1/user_register", func(c *gin.Context) {
				API_PostUserRegister(c)
			})
		*/
